...
void *doConsumerThing( void *tid ) 
{
	...
	if ( cc <= 0 )
	{
		fprintf( stderr, "[ERROR] [write CONSUME] Connection lost.\n" );
		close( csock ); 
		pthread_exit( NULL );
	}
	...
	if ( cc <= 0 )
	{
		fprintf( stderr, "[ERROR] [read int] Connection lost.\n" );
		close( csock ); 
		pthread_exit( NULL );
	}
	...
	if ( cc <= 0 )
	{
		fprintf( stderr, "[ERROR] [read stream of chars] Connection lost.\n" );
		close( csock ); 
		pthread_exit( NULL );
	}
