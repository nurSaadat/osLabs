...
void *doProducerThing(void *tid)
{
	...
	sprintf(message, "[ERROR] [write PRODUCE] Connection lost.\n");
	if ( cc <= 0 )
		threadErrorExit(csock, message);
	...
	sprintf(message, "[ERROR] [recieve response] Connection lost.\n");
	if ( cc <= 0 )
		threadErrorExit(csock, message);
	...
	sprintf(message, "[ERROR] [send int] Connection lost.\n");
	if ( cc <= 0 )
		threadErrorExit(csock, message);
	...
	sprintf(message, "[ERROR] [send char stream] Connection lost.\n" );
	if ( cc <= 0 )
		threadErrorExit(csock, message);

	cc = read( csock, buf, BUFSIZE );
	sprintf(message, "[ERROR] [receive DONE] Connection lost.\n" );
	if ( cc <= 0 )
		threadErrorExit(csock, message);
