...
main( int argc, char *argv[] )
{
	...
	/* Create an array of size num_cons
...
void *doConsumerThing( void *tid ) 
{
	...
	{	
		...
		if (cc <= 0)
		{				
			fprintf( stderr, "[ERROR] [%ld] Error on streaming.\n", pthread_self() );	
			free(str);
			break;
		}
	...
	char number_token[12];
	sprintf(number_token, " %d", check);
	...
	write( dstf, number_token, strlen(number_token)); 		
